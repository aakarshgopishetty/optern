...
export class CandidateSignUp {
  ...
  private validateForm(): string | null {
    ...
    if (!this.model.university || this.model.university.trim() === '') {
      return 'University is required.';
    }
    if (!this.model.course || this.model.course.trim() === '') {
    ...
    // University and course validation (minimum length)
    if (this.model.university.trim().length < 2) {
      return 'University name must be at least 2 characters long.';
    }
    if (this.model.course.trim().length < 2) {
